...
(defmacro profile
  "性能分析宏：测量代码块执行时间
   ...
   ```"
  [name & body]
  `(let [namespaced-name# (keyword (str *ns*) (clojure.core/name ~name))
...
(defn profile-fn
  ...
  [name f & args]
  (let [;; 从调用栈推断命名空间
        caller-ns (or (some-> (Thread/currentThread)
                             .getStackTrace
                             (aget 2)
                             .getClassName
                     ...
                     "unknown")
        namespaced-name (keyword caller-ns (clojure.core/name name))
        start-time (System/nanoTime)
        result (apply f args)
        end-time (System/nanoTime)
        duration (- end-time start-time)]
