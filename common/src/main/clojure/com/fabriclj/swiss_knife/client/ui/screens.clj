(ns com.fabriclj.swiss-knife.client.ui.screens
  "瑞士军刀 - GUI/屏幕系统模块

   提供自定义 GUI 屏幕（Screen）的创建和管理功能。
   注意：
   - 此命名空间仅在客户端环境可用！
   - 不要与 common.inventories 中的容器GUI（AbstractContainerMenu）混淆
   - 这里是全屏菜单界面，而容器GUI是方块交互界面
   原名: client.menus（已重命名以避免与容器菜单混淆）"
