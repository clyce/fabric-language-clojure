(ns com.fabriclj.swiss-knife.client.hud
  "瑞士军刀 - HUD 渲染模块
...
(defn get-screen-center
  "获取屏幕中心坐标
   ...
   返回：{:x x :y y}"
  []
  (let [{:keys [width height]} (render/get-window-size)]
    {:x (/ width 2)
     :y (/ height 2)}))
...
(defn get-top-right
  ...
  ([]
   (get-top-right 5))
  ([margin]
   (let [{:keys [width]} (render/get-window-size)]
...
(defn get-bottom-left
  ...
  ([]
   (get-bottom-left 5))
  ([margin]
   (let [{:keys [height]} (render/get-window-size)]
