(ns com.fabriclj.swiss-knife.common.worldgen
  "世界生成系统
...
(defn ore-configuration
  "创建矿石配置
   ...
   生成算法：
   Minecraft 的矿石生成使用椭球算法。每次尝试生成时，会在目标位置创建一个
   椭球形矿脉，矿脉中的每个方块都有 discard-chance 的概率被丢弃，从而形成
   不规则的矿脉形状。
...
(defn heightmap-placement
  "基于高度图放置（地表/海底/等）
   ...
   参数:
   - type: 高度图类型（:motion_blocking/:ocean_floor/:world_surface）
